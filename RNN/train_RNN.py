...
This script implements the training process for the StandardRNN model using PPO (Proximal Policy Optimization).
It includes:
- Data loading and preprocessing, including embedding computation.
- Model initialization for StandardRNN, PPOAgent, and auxiliary components.
...
Functions:
- train_rnn(config):
  Trains an RNN-based model with PPO reinforcement learning, performs validation, and evaluates model performance.
"""
...
from models.RNN import StandardRNN
...
import numpy as np
from models.RNN import validate_model_rnn,test_model_rnn,custom_reward_function
...
def train_rnn(config):
    ...
    hidden_size = config.get("hidden_size", 192)
    action_dim = 2
    input_size = config.get("input_size", 64)  # Example: dataset.num_classes
    ...
    for _ in range(n_runs):
        ...
        rnn_s = StandardRNN(input_size, hidden_size).to(device)
        mlp_transform_s = StateTransformMLP(action_dim, hidden_size).to(device)
        agent_s = PPOAgent(hidden_size, action_dim, rnn_s, mlp_transform_s).to(device)
        memory = Memory()
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, (batch_seqs, batch_labels) in enumerate(train_loader):
                ...
                hidden_states = rnn_s.process_sequence(all_inputs)
            ...
            val_reward, val_accuracy, val_precision, val_recall, val_f1 = validate_model_rnn(
                agent_s, rnn_s, mlp_transform_s, val_loader, target_model, data
            )
            ...
            if val_reward > best_val_reward:
                ...
                torch.save({
                    ...
                    'rnn_state_dict': rnn_s.state_dict(),
                    'mlp_transform_state_dict': mlp_transform_s.state_dict(),
                    'epoch': epoch + 1,
                    'best_val_reward': best_val_reward
                }, f"best_rnn_model_{lamb}.pth")

        accuracy, precision, recall, f1, auc_value = test_model_rnn(
            agent_s, rnn_s, mlp_transform_s, test_loader, target_model, data, device
        )
